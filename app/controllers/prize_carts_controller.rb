class PrizeCartsController < ApplicationController
  ...
  def index
    @prize_carts = @active_event.prize_carts.includes(:scout).where("scouts.is_admin = ?", false).order("scouts.last_name ASC").page(params[:page]).per(40)
  end
  ...
  def approved_prizes
    @approved_prizes = @active_event.prize_carts.approved.joins(cart_prizes: :prize).order("prizes.source, prizes.sales_amount").group('cart_prizes.prize_id', 'prizes.source', 'prizes.sales_amount').sum('cart_prizes.quantity')
  end
