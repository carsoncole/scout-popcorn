class Stock < ApplicationRecord
  ...
  def self.wholesale_value(unit, event)
    value = 0
    unit.stocks.joins(:product).where("products.is_physical_inventory = ? AND products.is_pack_donation = ?",true, false).group(:product_id).sum(:quantity).each do |product_id,quantity|
